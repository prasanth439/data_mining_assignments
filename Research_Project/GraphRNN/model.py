...
class GRU_plain(nn.Module):
    ...
    def forward(self, input_raw, pack=False, input_len=None, get_hidden=False):
        if self.has_input:
            input = self.input(input_raw)
            input = self.relu(input)
        else:
        ...
        if pack:
            input = pack_padded_sequence(input, input_len, batch_first=True)
        output_raw_raw, self.hidden = self.rnn(input, self.hidden)
        if pack:
            output_raw_raw = pad_packed_sequence(output_raw_raw, batch_first=True)[0]
        if self.has_output:
            output_raw = self.output(output_raw_raw)
        # return hidden state at each time step
        if get_hidden:
            return output_raw, output_raw_raw
        return output_raw
