...
set<string> old_frequent_itemsets;
int MIN_SUPPORT = 1;
...
void flush_freq(int mode,int k){
    for (auto it=old_frequent_itemsets.begin(); it != old_frequent_itemsets.end(); ++it){
      if(mode){
        ofle << *it << "\n";
      }  
      if(k!=1){
...
bool check_Ksubstrings(string &temp,int k){

    vector<int> spaceind;
    for(int i =0;i< temp.size();i++){
        if(temp[i] ==' '){
            spaceind.push_back(i);
        }
    }
    string temp2;
    for (int i = 0; i < spaceind.size()-1; i++)
    {
        temp2 = temp.substr(0,spaceind[i]+1)+temp.substr(spaceind[i+1]+1);
        if(frequent_itemset_count.find(temp2)==frequent_itemset_count.end()){
            cout<< "Ksub "<< k <<" "<<temp2 << endl;
            return false;
        }
    }
    temp2 = temp.substr(spaceind[0]+1);
    if(frequent_itemset_count.find(temp2)==frequent_itemset_count.end()){
        return false;
    }
    return true;   
...
void parseFile(string path)
{
    ...
    do {
        if (k == 1){
            ...
            old_frequent_itemsets = frequent_itemsets;
